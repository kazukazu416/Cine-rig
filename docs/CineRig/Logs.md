...
**対応ファイル:** src/exportPdf.ts（新規）, src/App.tsx, src/index.css

html2canvas + jsPDF を使用した A4横 2ページ PDF 書き出し機能を実装。

**1ページ目:** ヘッダー（CineRigロゴ・案件名・担当者・日付） + 配線図（70%）+ フッター
**2ページ目:** ケーブル一覧（表形式）+ バッテリー計算結果（カメラグループ・モニター・合計本数）+ フッター

**UI変更:** トップバーに「PDF を書き出す」ボタン追加。書き出し中はスピナー表示。
